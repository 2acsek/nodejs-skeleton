...
import { gracefulWrapperFactory, consumerFactory, serverFactory } from './graceful-stop.factory';
...
describe('runWithTimeout', () => {
  ...
  test('should not reject on non-timeout', async () => {
    const res = await runWithTimeout(100, delay(1));
...
describe('gracefulWrapperConsumerFactory', () => {
  ...
  test('should initiate consumer shutdown', async () => {
    const consumer = consumerFactory({ isProcessing: async () => false });
